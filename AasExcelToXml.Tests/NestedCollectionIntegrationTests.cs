...
public sealed class NestedCollectionIntegrationTests
{
    ...
    [Fact]
    public void Convert_WithMultiLevelCollectionPath_PreservesThreeLevelHierarchy_Aas2()
    {
        var path = Path.Combine(Path.GetTempPath(), $"nested-collection-3-{Guid.NewGuid():N}.xlsx");
        var outputPath = Path.Combine(Path.GetTempPath(), $"aas-collection-3-v2-{Guid.NewGuid():N}.xml");
        ...
        using (var wb = new XLWorkbook())
        {
            ...
            ws.Cell(2, 3).Value = "Spec>Dim>Advanced";
        ...
        try
        {
            Converter.Convert(path, outputPath, "사양시트", new ConvertOptions { Version = AasVersion.Aas2_0 });

            var doc = XDocument.Load(outputPath);
            XNamespace ns = "http://www.admin-shell.io/aas/2/0";
            var advancedCollection = doc.Descendants(ns + "submodelElementCollection")
                .FirstOrDefault(x => string.Equals(x.Element(ns + "idShort")?.Value, "Advanced", StringComparison.Ordinal));

            Assert.NotNull(advancedCollection);
            Assert.Contains(advancedCollection!.Descendants(ns + "property"), p => string.Equals(p.Element(ns + "idShort")?.Value, "Mass", StringComparison.Ordinal));
        }
        finally
        {
            File.Delete(path);
